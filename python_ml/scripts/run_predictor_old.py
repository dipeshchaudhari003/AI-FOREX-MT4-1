...
def load_mt4_data():
    """
    Load latest MT4 candle data and build ML features.
    Returns:
        DataFrame
    """

    if not safe_copy(MT4_CSV, SAFE_CSV):
        raise Exception("MT4 file locked")
...
while True:
    try:
        ...
        # print(f"Threshold            : {conf_threshold:.2%}")
        ...
        # print(f"Prediction Edge      : {edge:.2%}")
        ...
        print(f"Time                 : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        #print(f"Closed Candle        : {closed_dt}")
        ...
        # print(f"Market Range         : {market_range:.2f}")

        # print(f"Prediction Edge      : {prediction_edge:.2f}")

        # print(f"Dynamic TP           : {dynamic_tp}")
        ...
        #print(f"Take Profit          : {dynamic_tp}")

        print(f"MT4 Files            : Updated Successfully")
