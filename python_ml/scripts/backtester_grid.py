"""
Grid search over TP/SL using a model, includes spread and slippage, reports USD P&L per lot.
"""
import os
import joblib
import argparse
...
def simulate_trades(df, signals, tp_points=30, sl_points=300, max_holding=10, point=0.01, spread=2.0, slippage=1.0, lot_size=0.01):
    trades = []
    n = len(df)
    for i in range(len(signals)):
        ...
        if win is None:
            ...
            if sig == 1:
                ...
                pnl_points = (entry_price - exit_price) / point
        # convert to USD assuming 1 point value per standard lot = 100 * point
        usd_per_point_per_std_lot = 100 * point
        usd_pnl = pnl_points * usd_per_point_per_std_lot * lot_size
        trades.append({'entry_index': entry_index, 'exit_index': exit_index, 'signal': sig, 'pnl_points': pnl_points, 'usd_pnl': usd_pnl, 'win': win})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tp_list', default='10,20,30,40,50')
    parser.add_argument('--sl_list', default='100,200,300,400,500')
    ...
    tp_list = [int(x) for x in args.tp_list.split(',')]
    sl_list = [int(x) for x in args.sl_list.split(',')]
    ...
    for tp in tp_list:
        for sl in sl_list:
            trades = simulate_trades(df, signals, tp_points=tp, sl_points=sl, max_holding=args.max_holding, point=args.point, spread=args.spread, slippage=args.slippage, lot_size=args.lot_size)
            if trades.empty:
                total_usd = 0.0
            else:
    ...
    print('\nGrid results (tp, sl, trades, total_usd):')
    for r in results:
        print(r)
    print('\nBest: ', best)
